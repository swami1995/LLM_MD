...
def initialize_all_auditors(trust_market_system, args, gemini_api_key, openai_api_key):
    auditors = []
    auditor_versions_to_add = ["main", "no_feedback"]#, "heuristic", "direct_investing", "nonbayesian_update"]

    if "main" in auditor_versions_to_add:
        auditor = AuditorWithProfileAnalysis(
            ...
            config_params={
                ...
                'use_direct_LLM_predictions': False,
    ...
    # Add Auditor without feedback context
    if "no_feedback" in auditor_versions_to_add:
        auditor_no_feedback = AuditorWithProfileAnalysis(
            source_id="auditor_no_feedback",
            market=trust_market_system.trust_market, # Pass the market instance
            api_key=gemini_api_key if args.llm_source == "api" else None,
            api_model_name=args.api_model_name if args.llm_source == "api" else None,
            api_provider=args.api_provider,
        ...
        # Provide profiles *after* simulation module registration (which copies profiles to market sys)
        for agent_id, profile in trust_market_system.agent_profiles.items():
            auditor_no_feedback.add_agent_profile(agent_id, profile)
        trust_market_system.register_auditor(auditor_no_feedback, evaluation_frequency=args.auditor_frequency)
        print(f"Registered Auditor: {auditor_no_feedback.source_id} (Eval Freq: {args.auditor_frequency})")
        auditors.append(auditor_no_feedback)

    # Add Auditor with heuristic (no optimizer)
    if "heuristic" in auditor_versions_to_add:
        auditor_heuristic = AuditorWithProfileAnalysis(
            source_id="auditor_heuristic",
            market=trust_market_system.trust_market, # Pass the market instance
            api_key=gemini_api_key if args.llm_source == "api" else None,
            api_model_name=args.api_model_name if args.llm_source == "api" else None,
            api_provider=args.api_provider,
        ...
        # Provide profiles *after* simulation module registration (which copies profiles to market sys)
        for agent_id, profile in trust_market_system.agent_profiles.items():
            auditor_heuristic.add_agent_profile(agent_id, profile)
        trust_market_system.register_auditor(auditor_heuristic, evaluation_frequency=args.auditor_frequency)
        print(f"Registered Auditor: {auditor_heuristic.source_id} (Eval Freq: {args.auditor_frequency})")
        auditors.append(auditor_heuristic)
    
    if 'direct_investing' in auditor_versions_to_add:
        auditor_direct = AuditorWithProfileAnalysis(
            source_id="auditor_direct",
            market=trust_market_system.trust_market, # Pass the market instance
            api_key=gemini_api_key if args.llm_source == "api" else None,
            api_model_name=args.api_model_name if args.llm_source == "api" else None,
            api_provider=args.api_provider,
            ...
            config_params={
                'optimizer_enabled': False,
                'use_additional_context': True,
                'use_direct_LLM_predictions': True,
                'use_bayesian_updates': True,
            }
            # Pass llm_client if you initialize it separately
        )
        # Provide profiles *after* simulation module registration (which copies profiles to market sys)
        for agent_id, profile in trust_market_system.agent_profiles.items():
            auditor_direct.add_agent_profile(agent_id, profile)
        trust_market_system.register_auditor(auditor_direct, evaluation_frequency=args.auditor_frequency)
        print(f"Registered Auditor: {auditor_direct.source_id} (Eval Freq: {args.auditor_frequency})")
        auditors.append(auditor_direct)
    
    if 'nonbayesian_update' in auditor_versions_to_add:
        auditor_nonbayesian = AuditorWithProfileAnalysis(
            source_id="auditor_nonbayesian",
            market=trust_market_system.trust_market, # Pass the market instance
            api_key=gemini_api_key if args.llm_source == "api" else None,
            api_model_name=args.api_model_name if args.llm_source == "api" else None,
            api_provider=args.api_provider,
            ...
            config_params={
                ...
                'use_additional_context': True,
                'use_direct_LLM_predictions': False,
                'use_bayesian_updates': False,
            }
            # Pass llm_client if you initialize it separately
        )
        # Provide profiles *after* simulation module registration (which copies profiles to market sys)
        for agent_id, profile in trust_market_system.agent_profiles.items():
            auditor_nonbayesian.add_agent_profile(agent_id, profile)
        trust_market_system.register_auditor(auditor_nonbayesian, evaluation_frequency=args.auditor_frequency)
        print(f"Registered Auditor: {auditor_nonbayesian.source_id} (Eval Freq: {args.auditor_frequency})")
        auditors.append(auditor_nonbayesian)
    return auditors

def initialize_all_user_reps(trust_market_system, args, gemini_api_key, openai_api_key):
    user_reps = []
    user_rep_versions_to_add = ["main", "no_feedback"]#, "heuristic", "direct_investing", "nonbayesian_update"]
    # Add User Representatives
    if "main" in user_rep_versions_to_add:
    ...
    # Add User Representatives without feedback context
    if "no_feedback" in user_rep_versions_to_add:
        user_rep_no_feedback = UserRepresentativeWithHolisticEvaluation(
            source_id="user_rep_no_feedback",
            user_segment="balanced", # Define segment characteristics if needed
            representative_profile={}, # Can add details about the rep's focus
            market=trust_market_system.trust_market,
            api_key=gemini_api_key if args.llm_source == "api" else None,
        ...
        # The representative needs to know which user IDs it's responsible for
        for user_idx in range(customer_support_sim.num_users):
            user_rep_no_feedback.add_represented_user(user_idx, trust_market_system.user_profiles[user_idx]) # user_idx matches simulation's internal ID
        trust_market_system.register_user_representative(user_rep_no_feedback, evaluation_frequency=args.user_rep_frequency)
        print(f"Registered User Representative: {user_rep_no_feedback.source_id} (Eval Freq: {args.user_rep_frequency})")
        user_reps.append(user_rep_no_feedback)

    # Add User Representatives with heuristic (no optimizer)
    if "heuristic" in user_rep_versions_to_add:
        user_rep_heuristic = UserRepresentativeWithHolisticEvaluation(
            source_id="user_rep_heuristic",
            user_segment="balanced", # Define segment characteristics if needed
            representative_profile={}, # Can add details about the rep's focus
            market=trust_market_system.trust_market,
            api_key=gemini_api_key if args.llm_source == "api" else None,
        ...
        # The representative needs to know which user IDs it's responsible for
        for user_idx in range(customer_support_sim.num_users):
            user_rep_heuristic.add_represented_user(user_idx, trust_market_system.user_profiles[user_idx]) # user_idx matches simulation's internal ID
        trust_market_system.register_user_representative(user_rep_heuristic, evaluation_frequency=args.user_rep_frequency)
        print(f"Registered User Representative: {user_rep_heuristic.source_id} (Eval Freq: {args.user_rep_frequency})")
        user_reps.append(user_rep_heuristic)
    
    if 'direct_investing' in user_rep_versions_to_add:
        user_rep_direct = UserRepresentativeWithHolisticEvaluation(
            source_id="user_rep_direct",
            user_segment="balanced", # Define segment characteristics if needed
            representative_profile={}, # Can add details about the rep's focus
            market=trust_market_system.trust_market,
            api_key=gemini_api_key if args.llm_source == "api" else None,
        ...
        # The representative needs to know which user IDs it's responsible for
        for user_idx in range(customer_support_sim.num_users):
            user_rep_direct.add_represented_user(user_idx, trust_market_system.user_profiles[user_idx]) # user_idx matches simulation's internal ID
        trust_market_system.register_user_representative(user_rep_direct, evaluation_frequency=args.user_rep_frequency)
        print(f"Registered User Representative: {user_rep_direct.source_id} (Eval Freq: {args.user_rep_frequency})")
        user_reps.append(user_rep_direct)

    if 'nonbayesian_update' in user_rep_versions_to_add:
        user_rep_nonbayesian = UserRepresentativeWithHolisticEvaluation(
            source_id="user_rep_nonbayesian",
            user_segment="balanced", # Define segment characteristics if needed
            representative_profile={}, # Can add details about the rep's focus
            market=trust_market_system.trust_market,
            api_key=gemini_api_key if args.llm_source == "api" else None,
        ...
        # The representative needs to know which user IDs it's responsible for
        for user_idx in range(customer_support_sim.num_users):
            user_rep_nonbayesian.add_represented_user(user_idx, trust_market_system.user_profiles[user_idx]) # user_idx matches simulation's internal ID
        trust_market_system.register_user_representative(user_rep_nonbayesian, evaluation_frequency=args.user_rep_frequency)
        print(f"Registered User Representative: {user_rep_nonbayesian.source_id} (Eval Freq: {args.user_rep_frequency})")
        user_reps.append(user_rep_nonbayesian)
    return user_reps

def initialize_all_regulators(trust_market_system, args, gemini_api_key, openai_api_key):
    regulator = Regulator(
        source_id="regulator",
        market=trust_market_system.trust_market,
        api_key=gemini_api_key if args.llm_source == "api" else None,
...
def detailed_prints_and_logging_saving(trust_market_system, args, all_detailed_evaluations=None):
    print("\n=== Simulation Complete ===")
...
def get_args():
    ...
    return args, gemini_api_key, openai_api_key, static_knowledge_base

def load_profiles_and_prompts(args, static_knowledge_base):
    ...
    print(user_indices, sampled_indices)
    # ipdb.set_trace()
    ...
    # ipdb.set_trace()
    return agent_profiles, user_profiles, conversation_prompts, static_knowledge_base

## main function
if __name__ == "__main__":
    args, gemini_api_key, openai_api_key, static_knowledge_base = get_args()
    # --- Load Profiles & Prompts ---
    print("Loading agent and user profiles...")
    agent_profiles, user_profiles, conversation_prompts, static_knowledge_base = load_profiles_and_prompts(args, static_knowledge_base)
    ...
    # Add Auditors
    auditors = initialize_all_auditors(trust_market_system, args, gemini_api_key, openai_api_key)
    # Add User Representatives
    user_reps = initialize_all_user_reps(trust_market_system, args, gemini_api_key, openai_api_key)
    # Add Regulator
    regulators = initialize_all_regulators(trust_market_system, args, gemini_api_key, openai_api_key)
    ...
    # --- 7. Final Summary ---
    detailed_prints_and_logging_saving(trust_market_system, args, all_detailed_evaluations=all_detailed_evaluations)
